...
// import Basicpg from './Basicpg.jsx';
...
const LandingPage = () => {
  ...
  const [loginState, setChange] = useState(false);
  const HandleClick = (e) => {

    if (localStorage.getItem('role')=='user') {
      // window.open("https://wa.me/919999999999?text=Hi%20there!%20I'm%20interested%20in%20your%20PG.", "_blank")
      navigate('/homepage', { replace: true });
      // navigate("/details", { state: { pg } });
    }
    else {
    ...
    <div className="landing-page">
      <div className="landing-page-content">
        ...
        <section style={{ backgroundColor: '#f7f7f7' }}>
          {/* <Basicpg/> */}
          <h2 style={{ textAlign: 'center' }}>Today Special </h2>
          <ul className="pg-list">

            {pgList.map((pg, index) => (
              <li key={index}>
                {/* <strong>{pg.name}</strong> — {pg.location} */}
                <div className='renderPg' >
                  <div>
                    <div className="pg-card" style={{ backgroundColor: 'rgb(247 241 236)' }}>
                      <img
                        src={pg.imageUrl || "D:\FInalSubProject\CDAC_PROJECT_KD_J028-main\CDAC_PROJECT_KD_J028-main\StayHub\stayhub\src\images\detailspagecrousel5.png"} // fallback image
                        alt={pg.name}
                        className="pg-image"
                      />
                      <div className="pg-details">
                        <h3>{pg.name}</h3>
                        <p>{pg.location}</p>
                        <p>Owner Name: {pg.owner.firstName}</p>

                        <p className="pg-type">Lodge</p>
                        {/* <p className="pg-desc">A scenic lodge nestled amidst the Canadian wilderness.</p> */}
                        <p><strong>Price per Night: ₹{pg.rent || 399}</strong></p>
                        <div className="pg-stars">⭐ ⭐ ⭐ ⭐</div>
                        <button className="pg-btn" onClick={HandleClick}>Book</button>
                      </div>
                      <button className="pg-btn1" style={{marginLeft: '15px',marginBottom:'20px'}}
                        onClick={() =>
                        window.open("https://wa.me/8959697952?text=Hi%20there!%20I'm%20interested%20in%20your%20PG.", "_blank")    
                        }
                      >
                        Chat with Owner
                      </button>
